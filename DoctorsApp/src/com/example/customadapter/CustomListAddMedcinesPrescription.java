...
public class CustomListAddMedcinesPrescription extends
		ArrayAdapter<ListDataMedicines> {

	private final Context context;
	private final ArrayList<ListDataMedicines> modelsArrayList;

	public CustomListAddMedcinesPrescription(Context context,
			ArrayList<ListDataMedicines> modelsArrayList) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (!modelsArrayList.get(position).isGroupHeader()) {
			rowView = inflater.inflate(R.layout.new_medicine_prescription,
					parent, false);

			// 3. Get icon,title & counter views from the rowView
			TextView medcineName = (TextView) rowView
					.findViewById(R.id.textViewMedicinename);
			TextView medicineDosage = (TextView) rowView
					.findViewById(R.id.textViewMedicineDosage);

			// 4. Set the text for textView
			// imgView.setImageResource(modelsArrayList.get(position).getIcon());
			medcineName
					.setText(modelsArrayList.get(position).getMedicineName());
			medicineDosage.setText(modelsArrayList.get(position)
					.getMedicineDosage());
		} else {
