...
public class Final_new_Prescription extends Activity {
		...
		case R.id.add_print:
			...
			if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.KITKAT) {
				Toast.makeText(getApplicationContext(),
						"This feature needs Android 4.4 or above",
			...
			// Start a print job, passing in a PrintDocumentAdapter
			// implementation
			// to handle the generation of a print document
			printManager.print(jobName, new PrintAdapter(
	...
	private ArrayList<ListDataMedicines> generateData() {
		...
		List<Map<String, String>> parameters = objParameterModel
				.getparameters();
		...
		/*
		 * Add elements array wise so if any array is empty it will not to be
		 * added into list
		 */

		// --------------Send Medicines array --------

		if (medicines.size() != 0 && models.size() == 0) {
			...
			if (parameters.size() != 0) {
				...
				for (int i = 0; i < parameters.size(); i++) {
					models.add(new ListDataMedicines(0, parameters.get(i).get(
							"parametername"), parameters.get(i).get(
							"parametervalue")));
				}
			}
		}
