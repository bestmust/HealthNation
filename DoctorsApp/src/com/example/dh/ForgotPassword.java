...
public class ForgotPassword extends Activity implements OnClickListener {
	...
	private void init() {
		...
		Typeface sofiaPro = Typeface.createFromAsset(getAssets(),
				"SofiaProLight.otf");
	...
	public void onClick(View v) {
		...
		case R.id.buttonForgotpassword:
			...
			if (editTextEmail.getText().toString().equalsIgnoreCase("")) {
				ErrorDialog.ErrorDialogCreation(ForgotPassword.this, "Warning",
						"Please Enter Email");
			} else {

				if (!NetStatus.getInstance(getBaseContext()).isOnline(
						getBaseContext())) {
				...
				objForgotPasswordModel.setEmail(editTextEmail.getText()
						.toString());
				new ForgotPasswordTask(ForgotPassword.this)
						.execute(objForgotPasswordModel);
			}
