...
public class PatientsProfileTask extends
	...
	protected void onPostExecute(String result) {
		...
		if (response.success == 1) {
			...
			Editor ed = sp.edit();
			ed.putString(activity.getString(R.string.sp_patient_name),
					response.user.name);
			ed.putInt(activity.getString(R.string.sp_patient_id),
					response.user.patient_id);
			ed.putInt(activity.getString(R.string.sp_patient_person_id),
					response.user.person_id);
			ed.apply();
			...
			patientsId = (EditText) activity
					.findViewById(R.id.editTextPatientsUserName);
	...
	protected String doInBackground(PatientsParameterModel... params) {
		...
		nameValuePair.add(new BasicNameValuePair("email",
				objPatientsParameterModel.getPatientsId()));
